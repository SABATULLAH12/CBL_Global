...
function DefaultSelections() {
    $("div.filter-data-popup").each(function (id, val) {
        if (!($(this).parent().find('.filter-optional-title').length) && !(pageId == 3 && $(this).attr("filter-data-type") == "Brands/Category"))
            $(this).find(".filter-level").last().find("[button-type='radio'],[button-type='checkbox']").first().click();
    });
